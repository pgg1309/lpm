...
plot_charts("44W72", "RETAIL & FOOD SERVICES(EXCL. MOTOR VEH & PARTS AND GASOLINE STATIONS)", "T07046", FALSE)
```
...
###  Motor vechicle and parts dealers (prices)
Os 
```{r}
db <- cpi_goods %>% 
  pivot_wider(names_from = series_id, values_from = value) %>% 
...
# Estimate model with all ncomps
model <- plsr(value ~ ., ncomp = 20, data = select(db[idx,], -month), scale = TRUE, validation = "LOO")
validationplot(model)
# Estimate number of components
ncomp.onesigma <- selectNcomp(model, method = "onesigma", plot = TRUE)

# Estimate model with recommended number of components
model <- plsr(value ~ ., ncomp = ncomp.onesigma, data = select(db[idx,], -month), scale = TRUE, validation = "LOO")
 
# check fit
plot(model, ncomp = 1:ncomp.onesigma, asp = 1, line = TRUE)
drop(predict(model, newdata = db[!idx,]))[ncomp.onesigma]
...
# Estimate model with all ncomps
model <- plsr(value ~ ., ncomp = 50, data = select(db[idx,], -month), scale = TRUE, validation = "LOO")
validationplot(model)
# Estimate number of components
ncomp.onesigma <- selectNcomp(model, method = "onesigma", plot = TRUE)
...
plot_charts("443", "ELECTRONICS & APPLIANCE STORES", "T07011", FALSE)

db <- cpi_goods %>% 
  pivot_wider(names_from = series_id, values_from = value) %>% 
  full_join(
    price_data %>%
      filter(SeriesCode == "T07011") %>%
      transmute(month = yearmonth(TimePeriod), value = DataValue),
    by = 'month') %>% 
  mutate(across(-month, ~.x/lag(.x)-1)) %>% 
  filter_index("1992 Feb" ~.) %>% 
...
plot_charts("444", "BUILDING MATERIALS, GARDEN EQ. & SUPPLY DEALERS", "T07012")

db <- cpi_goods %>% 
  pivot_wider(names_from = series_id, values_from = value) %>% 
  full_join(
    price_data %>%
      filter(SeriesCode == "T07012") %>%
      transmute(month = yearmonth(TimePeriod), value = DataValue),
    by = 'month') %>% 
  mutate(across(-month, ~.x/lag(.x)-1)) %>% 
  filter_index("1992 Feb" ~.) %>% 
...
plot_charts("445", "FOOD & BEVERAGE STORES", "T07015")

db <- cpi_goods %>% 
  pivot_wider(names_from = series_id, values_from = value) %>% 
  full_join(
    price_data %>%
      filter(SeriesCode == "T07015") %>%
      transmute(month = yearmonth(TimePeriod), value = DataValue),
    by = 'month') %>% 
  mutate(across(-month, ~.x/lag(.x)-1)) %>% 
  filter_index("1992 Feb" ~.) %>% 
...
plot_charts("446", "HEALTH & PERSONAL CARE STORES", "T07027")
...
plot_charts("447", "GASOLINE STATIONS", "T07028")
...
plot_charts("448", "CLOTHING & CLOTHING ACCESSORIES STORES", "T07019")
...
```{r}
check_data("451", "SPORTING GOODS, HOBBY, MUSICAL INST. & BOOK STORES")
plot_charts("451", "SPORTING GOODS, HOBBY, MUSICAL INST. & BOOK STORES", "T07029")
```

## General merchandise stores
```{r}
check_data("452", "GENERAL MERCHANDISE STORES")
plot_charts("452", "GENERAL MERCHANDISE STORES", "T07023")
```

## Miscellaneous store retailers
```{r}
check_data("453", "MISCELLANEOUS STORES RETAILERS")
plot_charts("453", "MISCELLANEOUS STORES RETAILERS", "T07032")
```

## Nonstore retailers
```{r}
check_data("454", "NONSTORE RETAILERS", "T07037")
plot_charts("454", "NONSTORE RETAILERS", "T07037")
```

## Food services and drinking places
```{r}
check_data("722", "FOOD SERVICES & DRINKING PLACES", "T07041")
plot_charts("722", "FOOD SERVICES & DRINKING PLACES", "T07041")
```
