...
# prepare_eurodollar_data.R
ed <- read_rds("data/eurodollar.rds") # eurodollar

# prepare_fedfunds_data.R
...
```{r}
bind_rows(
  filter(ff_mod, day %in% as_date(c("2021-10-15", "2022-03-29", "2019-07-15", "2017-01-16"))) %>% mutate(type = 'ff') %>% filter(duration <= 548),
  filter(ed_mod, day %in% as_date(c("2021-10-15", "2022-03-29", "2019-07-15", "2017-01-16"))) %>% mutate(type = 'ed')) %>% 
  mutate(
    day_type = paste(type, day, sep="-"),
    q = duration / duration(3, "month"),
    premia = map2_dbl(q, type, spread_fit, spread_table),
...
bind_rows(
  ...
  theme(legend.position = 'bottom') +
  coord_cartesian(xlim = c(0, 14), ylim = c(-0.5, 2.75), expand = FALSE)
...
beta_4y <- splinefun(loadings_table$q, loadings_table$beta, 'natural')(16)
...
get_beta <- function(p, ltable = loadings_table) {
splinefun(ltable$q, ltable$beta, 'natural')(p)
}

# using only eurodollar 
ed_mod %>%
    filter(
      day %in% as_date(c("2021-10-15", "2022-03-29", "2019-07-15", "2018-09-17"))
      ) %>%
  mutate(type = 'ed') %>% 
  select(-name, -value) %>% 
  left_join(select(new_cash,day, new_basis), by = 'day') %>% 
  mutate(
    day_type = paste(type, day, sep="-"),
    q = duration / duration(3, 'month'),
    beta_q = map_dbl(q, get_beta),
    ed_4y  = map_dbl(day, rate_curve_fit, 1461,    ed_mod),
    ed_5y  = map_dbl(day, rate_curve_fit, 1826.25, ed_mod),
    premia = map2_dbl(q, type, spread_fit, recent_spread_table),
    eff_c  = rate - premia,
    eff_v  = rate + beta_q*(ed_4y - ed_5y) - new_basis
    ) %>% 
  select(day, q, eff_c, eff_v, day_type, type) %>% 
  pivot_longer(c(eff_c, eff_v)) %>% 
  mutate(name = if_else(name == "eff_c", "Constant term premium", "Time-varying term premium")) %>% 
  ggplot(
    aes(x=q, y=value, lty = name, color = as.character(day))) + 
  geom_line(size = 1) +
  geom_hline(yintercept = 0) +
  scale_x_continuous(
  ...
  labs(
    ...
    subtitle = "Under constant and variable risk premium",
    x = "quarters",
    y = "expected fed funds rate",
    color = "As of:",
    lty = "contract"
  ...
  theme(legend.position = 'bottom') +
  coord_cartesian(xlim = c(0, 20), ylim = c(-0.5, NA), expand = FALSE)
...
$$
fut_t^{ed}(2y)-\left(1+\beta^{ed}(4y) - \beta^{ed}(2y)\right).fut_t^{ed}(4y) + \left(\beta^{ed}(4y) - \beta^{ed}(2y)\right).fut_t^{ed}(5y) = \\
fut_t^{ed}(2y) - fut_t^{ed}(4y) + \beta^{ed}(2y)(fut_t^{ed}(4y) - fut_t^{ed}(5y)) -\beta^{ed}(4y)(fut_t^{ed}(4y)-fut_t^{ed}(5y) = \\
fut_t^{ed}(2y) - fut_t^{ed}(4y) - \beta^{ed}(2y).s_t + \beta^{ed}(4y).s_t = \\
fut_t^{ed}(2y) - fut_t^{ed}(4y) - \rho_t^{ed}(2y) + \rho_t^{ed}(4y) = \\
fut_t^{ed}(2y) - \rho_t^{ed}(2y) - (fut_t^{ed}(4y)-\rho_t^{ed}(4y)) = \\
E_t\left[\bar{ff}(2y)\right] + c_t - (ff_t^* - c_t) = \\
E_t\left[\bar{ff}(2y)\right] - ff_t^*
$$
...
tibble(day = trading_day, 
       ...
       ed_5y = fut_ed_5y) %>% 
  mutate(
    dev_1y_ff = ed_1y + (beta_4y - beta_1y)*ed_5y -(1+beta_4y-beta_1y)*ed_4y,
    dev_2y_ff = ed_2y + (beta_4y - beta_2y)*ed_5y -(1+beta_4y-beta_2y)*ed_4y,
    dev_3y_ff = ed_3y + (beta_4y - beta_3y)*ed_5y -(1+beta_4y-beta_3y)*ed_4y
    ) %>% 
  select(day, dev_1y_ff, dev_2y_ff, dev_3y_ff) %>% 
  pivot_longer(-day) %>% 
  ggplot(aes(x=day, y=value, color = name)) +
  geom_line() +
  ...
  labs(
    title = "Policy Expectations Measure (deviation from long-run)",
    y = "%",
    x = NULL,
    color = NULL
  )+
