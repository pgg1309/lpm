myaidsConcav <- function (priceNames, totExpName, coef, data, shareNames = NULL)
{
  ...
  if (isSymmetric(coef$gamma, tol = 1e-06, check.attributes = FALSE) !=
      TRUE) {
  ...
  for (t in 1:nObs) {
    result$cMatrices[[t]] <- coef$gamma + (coef$beta %*%
                                             t(coef$beta)) * (log(xt[t]) - lnp[t]) - diag(shareMat[t,
                                             ]) + shareMat[t, ] %*% t(shareMat[t, ])
    result$concavity[t] <- miscTools::isSemidefinite(result$cMatrices[[t]][1:(nGoods -
                                                                       1), 1:(nGoods - 1)], positive = FALSE)
  }
